from fuzzywuzzy import fuzz  # Импорт функции для расчета дистанции Левенштейна
...
def get_best_matches(inverted_index, query_word):
    ...
    for word, documents in inverted_index.items():
        # Расчет дистанции Левенштейна между запросом и словом в индексе
        score = fuzz.ratio(query_word, word)
        if score > worst_score: # Если результат лучше, то сохраняем найденные значения
            worst_score = score
            best_match = word
